...
一般情况下，首先单独生成重构型脚本进行调试，调试通过之后直接使用相同配置生成虚拟型脚本

发布虚拟型脚本
--------------

运行虚拟型脚本同样需要把依赖包 :term:`pyarmor.mini` 一起发布
...
生成嵌入型加密脚本也需要安装包 :term:`pyarmor.mini`::

  $ pip install pyarmor.mini

我们依旧使用上例中创建的工程，使用相同配置生成嵌入型脚本::
...
一般情况下，首先单独生成重构型脚本进行调试，调试通过之后直接使用相同配置生成嵌入型脚本

发布嵌入型脚本
--------------

运行嵌入型脚本同样需要把依赖包 :term:`pyarmor.mini` 一起发布
